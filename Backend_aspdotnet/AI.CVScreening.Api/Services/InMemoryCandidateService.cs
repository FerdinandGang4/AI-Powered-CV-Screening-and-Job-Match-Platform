...
public sealed class InMemoryCandidateService : ICandidateService
{
    ...
    public InMemoryCandidateService(AppMemoryStore store)
    {
        _store = store;

        if (_store.Candidates.Count == 0)
        {
            _store.Candidates.AddRange(
            [
                new()
                {
                    Id = Guid.NewGuid(),
    ...
    {
        return _store.Candidates.FirstOrDefault(candidate => candidate.Id == id);
    }
}
